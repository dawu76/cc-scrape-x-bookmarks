...
Injecting the script (CLAUDE.md Step 3) only runs the class constructor —
it builds `window.bookmarkInterceptor` but does not attach either hook.
`install()` must be called explicitly before `startAutoScroll()`, or
scrolling proceeds with zero captures and eventually trips the stall
watchdog (10 scrolls with no intercepted response). CLAUDE.md's Step 3 now
includes this call and a mandatory `isActive: true` verification.
